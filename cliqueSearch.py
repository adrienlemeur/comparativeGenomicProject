...
for clq in nx.clique.find_cliques(G): #la fonction find_cliques permet de renvoyer toutes les cliques maximales dans un graphe non oriente G
	print(len(clq))
	if len(clq) < 21: #si la clique ne fait pas intervenir tous les génomes, on les stocke dans un fichier à part (cliques_pas_max)
		for clq_item in clq:
			f.write(clq_item.decode("utf-8"))
			f.write("\t")
		f.write("\n")
	else :#cliques faisant intervenir tous les génomes
		for clq_item in clq:
